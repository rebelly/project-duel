...
namespace ConsoleApp1
{
    ...
    class Game
    {
        ...
        public static void Choise(out int[] choise, card[] deck1) // функция которая на выходе дает нам три выбранные карты одного из бойцов
        {
            int k = 0;
            choise = new int[3];
            string ch;
            ...
            while (k < 3)
            {
                Console.Write($"Выберите {k + 1} карту: "); 
                ch = Console.ReadLine();
                try
                {
                    if (int.TryParse(ch, out int j))
                    {
                        if (j < deck1.Length) 
                        {
                            ...
                            else if (k == 2 & h_char & !alm_dead) throw new Exception("Как вы себе персдтавляете битву доспехов без их хозяина, выберите карту персонажа");
        ...
        public static void print_deck(card[] deck)
        {
            for (int i = 0; i < deck.Length - 1; i++)
            {
                if (deck[i].is_char)
                {
                    ...
                    Console.ForegroundColor = ConsoleColor.Cyan;
                    Console.Write(deck[i].Name);
                    Console.ForegroundColor = ConsoleColor.White;
                    Console.Write(" Очки Здоровья: ");
                    Console.ForegroundColor = ConsoleColor.Cyan;
                    Console.Write(deck[i].Health);
                    Console.ForegroundColor = ConsoleColor.White;
                    Console.Write(" Очки Защиты: ");
                    Console.ForegroundColor = ConsoleColor.Cyan;
                    Console.Write(deck[i].Defence);
                    Console.ForegroundColor = ConsoleColor.White;
                    Console.Write(" Очки Атаки: ");
                    Console.ForegroundColor = ConsoleColor.Cyan;
                    Console.Write(deck[i].Attack);
                    Console.ForegroundColor = ConsoleColor.White;
                    Console.Write("\n");
                }
                else {
        ...
        public static card[] game(out card[] deck1)
        {  
        ...
        public static void stat(string str, out string name, out int health, out int attack, out int def)
        {
            ...
            attack = int.Parse(words[4]); // разобрать строку в инты
        }
...

    {
        ...
        static void Main()
        {
            ...
            Console.ForegroundColor = ConsoleColor.White;
            Console.Write("Среди карт, выбранных игроком должно быть ровно");
            Console.ForegroundColor = ConsoleColor.Cyan;
            Console.Write(" 2 ");
            Console.ForegroundColor = ConsoleColor.White;
            Console.Write("карты аммуниции и ");
            Console.ForegroundColor = ConsoleColor.Cyan;
            Console.Write(" 1 ");
            Console.ForegroundColor = ConsoleColor.White;
            Console.WriteLine("карта персонажа \n");
            Console.ForegroundColor = ConsoleColor.Green;
            Console.Write("НО:\n");
            Console.ForegroundColor = ConsoleColor.White;
            Console.Write("Если вы решаете отправить в бой 'голого' персонажа ЗАДОХЛИКА, то вам разрешается взять ");
            Console.ForegroundColor = ConsoleColor.Cyan;
            Console.Write("ТРИ");
            Console.ForegroundColor = ConsoleColor.White;
            Console.Write(" карты аммуници \n");
            Console.WriteLine();
            int[] choise_ang;
            ...
            }       // на этом моменте у нас есть конечный и точно правильный выбор трех карт обоих игроков
